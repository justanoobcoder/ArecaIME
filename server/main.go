...
var serverLogEnabled = envBool("ARECA_UINPUT_SERVER_LOG") ||
	envBool("ARECA_UINPUT_SERVER_DEBUG") ||
	envBool("OPENKEY_NONPREEDIT_SERVER_LOG") ||
...
func logEvent(format string, args ...any) {
	...
	log.Printf("[areca-uinput-server] "+format, args...)
}
...
func (d *uinputDevice) ensureOpen() error {
	...
	copy(setup.Name[:], []byte("Areca IME Uinput Server"))
...
func runPlan(ctx context.Context, out chan<- string, plan planCommand, uinput backspaceSender) {
	totalEvents := plan.backspaces + 1
	for i := 0; i < totalEvents; i++ {
		if i > 0 && !waitOrCancel(ctx, plan.interKey) {
			logEvent("cancelled before backspace session=%d tx=%d sent=%d/%d",
				plan.sessionID, plan.txID, i, totalEvents)
			return
		}
		kind := "backspace"
		...
		logEvent("inject %s session=%d tx=%d index=%d/%d",
			kind, plan.sessionID, plan.txID, i+1, totalEvents)
		if err := uinput.backspace(); err != nil {
			logEvent("uinput backspace failed session=%d tx=%d index=%d/%d err=%v",
				plan.sessionID, plan.txID, i+1, totalEvents, err)
			return
		}
		if ctx.Err() != nil {
			logEvent("cancelled after backspace session=%d tx=%d index=%d/%d",
				plan.sessionID, plan.txID, i+1, totalEvents)
			return
		}
	}
...
func main() {
	...
	priority := flag.Bool("priority",
		envBoolDefault("ARECA_UINPUT_SERVER_PRIORITY",
